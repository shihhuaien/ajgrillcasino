...
// 確認交易是否存在並檢查是否重複處理
export async function checkTransactionExists(transactionId, gameId, userId) {
  const { data, error } = await supabase
    .from("transaction")
    .select("transaction_id, game_id, user_id, settled")
  ...
  if (data.length > 0) {
    return { exists: true, settled: data[0].settled }; // 返回交易是否已結算的狀態
  }
